...
function slideAnimation() {
    gsap.to('.slide',{
        ...
        xPercent: -200,
        ease: "power2.inOut" // Adjust the ease value for a faster and smoother movement
    })
}
...
document.querySelectorAll('button').forEach(button => {
    button.addEventListener('click', function() {
        let buttonText = this.innerText; // Store the button text
        this.innerHTML = ''; // Clear the button text
        let chars = [...buttonText]; // Split the text into characters
        chars.forEach((char, index) => {
            setTimeout(() => {
                // Create a span for each character to individually style or animate
                let span = document.createElement('span');
                span.textContent = char;
                this.appendChild(span);
            }, index * 100); // Delay between each character appearing
        });
    });
});
}
